...
if (isset($_GET["publish"])) {
  $publishRecipeSQL = "UPDATE recipes SET status = '2' WHERE id = '$recipeId'";
  $link->query($publishRecipeSQL);
  showSuccessNotification("Рецепт опубликован!");
}
?>

<?php
if (isset($_GET["reject"])) {
  $publishRecipeSQL = "UPDATE recipes SET status = '3' WHERE id = '$recipeId'";
  $link->query($publishRecipeSQL);
  showSuccessNotification("Рецепт отклонен!");
}
?>
...
<section class="recipe">
  <div class="container">
    <?php
    $userRole = $user["role"];
    if ($recipe["status"] == '1' && ($userRole == '2' || $userRole == '3')) { ?>
    <div class="recipe_status">
      <p class="final_price_title">
        Рецепт находится на этапе модерации.
      </p>
      <div class="recipe_status_btns">
        <a class="modal-btn" href="?page=recipe&id=<?= $recipe["id"] ?>&publish"
          class="recipe_status_btn">Опубликовать</a>
        <a class="modal-btn modal-btn-success" href="?page=recipe&id=<?= $recipe["id"] ?>&reject"
          class="recipe_status_btn">Отклонить</a>
      </div>
    </div>
    <? }
    ?>
