...
$freeDeliveryPriceResponse = $link->query($getFreeDeliveryPriceSQL); $freeDeliveryPriceObject =
$freeDeliveryPriceResponse->fetch_assoc(); $freeDeliveryPrice = $freeDeliveryPriceObject["price"];
$getUserCartSQL = "SELECT * FROM cart WHERE userId = '$userId'"; $userCartResponse =
$link->query($getUserCartSQL); $userCart = $userCartResponse->fetch_assoc(); $getAllCartProductsSQL
= "SELECT * FROM cart_list WHERE cartId = '{$userCart['id']}'"; $cartProductsResponse =
$link->query($getAllCartProductsSQL); $totalPrice = 0; ?>

<?php
if (isset($_GET["deleteAll"])) {
  $deleteAllCartItemsSQL = "DELETE FROM cart_list WHERE cartId = '{$userCart['id']}'";
  $link->query($deleteAllCartItemsSQL); $deleteCartSQL = "DELETE FROM cart WHERE userId =
'$userId'"; $link->query($deleteCartSQL); redirect("page=cart"); } ?>
...
<div id="cartModal" class="modal">
    <div class="modal-content default-modal">
        ...
        <div class="modal-btns">
            ...
            <a id="closeCartModal" href="?page=cart&deleteAll" class="modal-btn modal-btn-success"
                >Удалить</a
            >
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div id="cart-content" class="content_cart">
            <div class="cart_products_list">
                <?php
        $cartProducts = $cartProductsResponse->fetch_assoc(); while ($cartProducts) { $productId =
                $cartProducts["productId"]; $getProductByIdSQL = "SELECT * FROM products WHERE id =
                '$productId'"; $productsResponse = $link->query($getProductByIdSQL); $product =
                $productsResponse->fetch_assoc(); $totalPrice += $product["price"]; ?>
                <div class="cart-product-item">
                    <img src="<?= $product["firstImage"] ?>" alt="<?= $product["name"] ?>"
                    class="image_cart_product">
          ...
          ?>
                ...
                <a href="?page=order&totalPrice=<?= $totalPrice ?>" class="button_cart"
                    >Перейти к оплате</a
                >
            </div>
        </div>
