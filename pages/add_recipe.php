...
if (isset($_POST["add-recipe"])) {
  ...
  if (empty($errors)) {
    ...
    $link->query($createNewRecipeSQL); $newRecipeId = $link->insert_id; $recipeProducts =
explode(";", $selectedProductId); if (is_array($recipeProducts)) { foreach ($recipeProducts as
$number) { $number = trim($number); $addRecipeProductSQL = "INSERT INTO recipe_products (recipeId,
productId) VALUES ('$newRecipeId', '$number')"; $link->query($addRecipeProductSQL); } }
showSuccessNotification("Рецепт успешно отправлен на рассмотрение"); } else {
showErrorNotifications($errors); } } ?>

<div id="rulesModal" class="modal">
    <div class="modal-content default-modal">
        <h3 class="modal-title">Правила публикации рецепта</h3>
        <ul class="rules_list">
            <li>
                <span>1. Уважительное поведение:</span> Просим всех пользователей проявлять уважение
                и толерантность во всех взаимодействиях на сайте. Запрещены оскорбления, угрозы,
                дискриминация или любое другое неприемлемое поведение.
            </li>
            <li>
                <span>2. Авторские права и оригинальность:</span> При публикации рецепта убедитесь,
                что вы являетесь владельцем содержимого или имеете разрешение владельца для его
                публикации. Не публикуйте чужие рецепты без разрешения или предоставляйте ссылку на
                источник, если используете рецепт из другого источника.
            </li>
            <li>
                <span>3. Точность и полнота:</span> При написании рецепта, старайтесь быть точными и
                полными. Предоставьте все необходимые детали, включая список ингредиентов, точные
                количества и пошаговые инструкции. Это поможет другим пользователям воспроизвести
                рецепт успешно.
            </li>
            <li>
                <span>4. Уникальность и оригинальность:</span> Пожалуйста, не публикуйте рецепты,
                которые уже есть на сайте, чтобы избежать дублирования контента. Приносите что-то
                новое и уникальное в сообщество.
            </li>
            <li>
                <span>5. Соблюдение законов и правил:</span> При написании и публикации рецептов,
                убедитесь, что они соответствуют местным законам и правилам, включая прописанные в
                области правильного питания.
            </li>
            <li>
                <span>6. Безопасность пищевых продуктов:</span> Убедитесь, что ваши рецепты
                соответствуют нормам безопасности пищевых продуктов. Предоставляйте рекомендации по
                хранению продуктов, безопасным способам приготовления и другим важным аспектам,
                связанным с безопасностью пищевых продуктов.
            </li>
            <li>
                <span>7. Модерация и отзывы:</span> Сайт оставляет за собой право модерировать и
                удалять любой контент, который не соответствует установленным правилам или может
                нанести вред другим пользователям.
            </li>
        </ul>
        <div class="modal-btns rules-btn">
            <button id="closeRulesModal" class="modal-btn modal-btn-cancel">Понятно</button>
        </div>
    </div>
</div>
...
<section class="form_admin">
    <div class="container">
        <div class="content_form">
            <h3 class="title_form">Добавление рецепта</h3>
            <button id="openRulesButton">Правила публикации рецепта</button>
            <form method="POST" class="form_style" name="add-recipe" enctype="multipart/form-data">
                ...
                <div class="input_label">
                    <label for="time" class="label_style"
                        >Введите время приготовления (в минутах)</label
                    >
                    <input
                        type="number"
                        class="input_style"
                ...
                <div>
                    <p class="caloric_value_input">На 100 граммов</p>
                    <div class="input_group">
                        <div class="input_label">
                            <label for="calories" class="label_style">Калории</label>
                            <input
                                type="text"
                                class="input_style_group"
                                name="calories"
                                value="<?= $calories ?>"
                                id="calories"
                            />
                ...
                <div class="input_label">
                    ...
                    <input
                        type="file"
                        class="input_style_image"
                        name="image"
                        value="<?= $image ?>"
                        id="image"
                    />
                </div>
                <button class="form_button_style" type="submit" name="add-recipe">Добавить</button>
            </form>
        </div>
    </div>
</section>
...
<script>
    ...
    addStepButton.addEventListener('click', (event) => {
        ...
        if (counter < 6) {
            counter++;
            stepsContainerElement.insertAdjacentHTML(
                'beforeend',
                `<div class="input_label">
...
<script>
    new Modal('rulesModal', 'openRulesButton', 'closeRulesModal');
</script>
