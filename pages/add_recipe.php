...
if (isset($_POST["add-recipe"])) {
  ...
  if (empty($errors)) {
    $userId = $user["id"];
    $createNewRecipeSQL = "INSERT INTO recipes (title, description, weight, calories, proteins, fats, carb, banner, complexity, time, status, userId, step1, step2, step3, step4, step5, step6) VALUES ('$title', '$description', '$weight', '$calories', '$proteins', '$fats', '$carb', '$banner', '$complexity', '$time', '1', '$userId', '$step1', '$step2', '$step3', '$step4', '$step5', '$step6')";
    $link->query($createNewRecipeSQL);
    $newRecipeId = $link->insert_id;
    $recipeProducts =
      explode(";", $selectedProductId);
    if (is_array($recipeProducts)) {
      foreach ($recipeProducts as
        $number) {
        $number = trim($number);
        $addRecipeProductSQL = "INSERT INTO recipe_products (recipeId,
productId) VALUES ('$newRecipeId', '$number')";
